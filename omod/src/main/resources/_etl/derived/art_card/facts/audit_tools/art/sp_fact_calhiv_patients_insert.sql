      ...
      WHERE mdp.voided = 0
        ...
        AND e.encounter_datetime >= DATE_SUB(CURRENT_DATE(), INTERVAL 12 MONTH)) cohort
         ...
         LEFT JOIN (SELECT client_id,
                    ...
                    GROUP BY client_id) return_date ON return_date.client_id = patient_id
         LEFT JOIN (SELECT client_id, MAX(encounter_date) AS latest_encounter_date, current_arv_regimen
                    FROM mamba_fact_encounter_hiv_art_card
                    WHERE current_arv_regimen IS NOT NULL
                    GROUP BY client_id) sub_current_arv_regimen ON sub_current_arv_regimen.client_id = patient_id
         LEFT JOIN (SELECT client_id, MAX(encounter_date) AS latest_encounter_date, adherence_assessment_code
                    FROM mamba_fact_encounter_hiv_art_card
                    WHERE adherence_assessment_code IS NOT NULL
                    GROUP BY client_id) sub_adherence_assessment_code
                   ON sub_adherence_assessment_code.client_id = patient_id
         LEFT JOIN (SELECT client_id, MAX(encounter_date) AS latest_encounter_date, arv_regimen_days_dispensed
                    FROM mamba_fact_encounter_hiv_art_card
                    WHERE arv_regimen_days_dispensed IS NOT NULL
                    GROUP BY client_id) sub_arv_regimen_days_dispensed
                   ON sub_arv_regimen_days_dispensed.client_id = patient_id
         LEFT JOIN (SELECT client_id, MAX(encounter_date) AS latest_encounter_date, hiv_viral_load, hiv_viral_load_date
                    FROM mamba_fact_encounter_hiv_art_card
                    WHERE hiv_viral_load IS NOT NULL
                    GROUP BY client_id) sub_hiv_viral_load ON sub_hiv_viral_load.client_id = patient_id
         LEFT JOIN (SELECT client_id, MAX(encounter_date) AS latest_encounter_date, hiv_viral_load_date
                    FROM mamba_fact_encounter_hiv_art_card
                    WHERE hiv_viral_load IS NULL
                      AND hiv_viral_load_date IS NOT NULL
                    GROUP BY client_id) sub_hiv_sample_collection ON sub_hiv_sample_collection.client_id = patient_id
         LEFT JOIN (SELECT client_id, MAX(encounter_date) AS latest_encounter_date, advanced_disease_status
                    FROM mamba_fact_encounter_hiv_art_card
                    WHERE advanced_disease_status IS NOT NULL
                    GROUP BY client_id) sub_advanced_disease_status
                   ON sub_advanced_disease_status.client_id = patient_id
         LEFT JOIN (SELECT client_id, MAX(encounter_date) AS latest_encounter_date, family_planning_status
                    FROM mamba_fact_encounter_hiv_art_card
                    WHERE family_planning_status IS NOT NULL
                    GROUP BY client_id) sub_family_planning_status ON sub_family_planning_status.client_id = patient_id
         LEFT JOIN (SELECT client_id, MAX(encounter_date) AS latest_encounter_date, nutrition_assesment
                    FROM mamba_fact_encounter_hiv_art_card
                    WHERE nutrition_assesment IS NOT NULL
                    GROUP BY client_id) sub_nutrition_assesment ON sub_nutrition_assesment.client_id = patient_id
         LEFT JOIN (SELECT client_id, MAX(encounter_date) AS latest_encounter_date, nutrition_support_and_infant_feeding
                    FROM mamba_fact_encounter_hiv_art_card
                    WHERE nutrition_support_and_infant_feeding IS NOT NULL
                    GROUP BY client_id) sub_nutrition_support_and_infant_feeding
                   ON sub_nutrition_support_and_infant_feeding.client_id = patient_id
         LEFT JOIN (SELECT client_id, MAX(encounter_date) AS latest_encounter_date, hepatitis_b_test___qualitative
                    FROM mamba_fact_encounter_hiv_art_card
                    WHERE hepatitis_b_test___qualitative IS NOT NULL
                    GROUP BY client_id) sub_hepatitis_b_test___qualitative
                   ON sub_hepatitis_b_test___qualitative.client_id = patient_id
         LEFT JOIN (SELECT client_id, MAX(encounter_date) AS latest_encounter_date, syphilis_test_result_for_partner
                    FROM mamba_fact_encounter_hiv_art_card
                    WHERE syphilis_test_result_for_partner IS NOT NULL
                    GROUP BY client_id) sub_syphilis_test_result_for_partner
                   ON sub_syphilis_test_result_for_partner.client_id = patient_id
         LEFT JOIN (SELECT client_id, MAX(encounter_date) AS latest_encounter_date, cervical_cancer_screening
                    FROM mamba_fact_encounter_hiv_art_card
                    WHERE cervical_cancer_screening IS NOT NULL
                    GROUP BY client_id) sub_cervical_cancer_screening
                   ON sub_cervical_cancer_screening.client_id = patient_id
         LEFT JOIN (SELECT client_id, MAX(encounter_date) AS latest_encounter_date, tuberculosis_status
                    FROM mamba_fact_encounter_hiv_art_card
                    WHERE tuberculosis_status IS NOT NULL
                    GROUP BY client_id) sub_tuberculosis_status ON sub_tuberculosis_status.client_id = patient_id
         LEFT JOIN (SELECT client_id, MAX(encounter_date) AS latest_encounter_date, tpt_status
                    FROM mamba_fact_encounter_hiv_art_card
                    WHERE tpt_status IS NOT NULL
                    GROUP BY client_id) sub_tpt_status ON sub_tpt_status.client_id = patient_id
         LEFT JOIN (SELECT client_id, MAX(encounter_date) AS latest_encounter_date, cd4_count
                    FROM mamba_fact_encounter_hiv_art_card
                    WHERE cd4_count IS NOT NULL
                    GROUP BY client_id) sub_cd4_count ON sub_cd4_count.client_id = patient_id
         LEFT JOIN (SELECT client_id, MAX(encounter_date) AS latest_encounter_date, crag_test_results
                    FROM mamba_fact_encounter_hiv_art_card
                    WHERE crag_test_results IS NOT NULL
                    GROUP BY client_id) sub_crag_test_results ON sub_crag_test_results.client_id = patient_id
         LEFT JOIN (SELECT client_id, MAX(encounter_date) AS latest_encounter_date, who_hiv_clinical_stage
                    FROM mamba_fact_encounter_hiv_art_card
                    WHERE who_hiv_clinical_stage IS NOT NULL
                    GROUP BY client_id) sub_who_hiv_clinical_stage ON sub_who_hiv_clinical_stage.client_id = patient_id
         LEFT JOIN (SELECT client_id, MAX(encounter_date) AS latest_encounter_date, differentiated_service_delivery
                    FROM mamba_fact_encounter_hiv_art_card
                    WHERE differentiated_service_delivery IS NOT NULL
                    GROUP BY client_id) sub_differentiated_service_delivery
         ...
         LEFT JOIN (SELECT client_id,
                           MAX(encounter_datetime) AS latest_encounter_date,
                           health_education_setting
                    FROM mamba_fact_encounter_hiv_art_health_education
                    WHERE health_education_setting IS NOT NULL
                    GROUP BY client_id) sub_health_education_setting
                   ON sub_health_education_setting.client_id = patient_id
         LEFT JOIN (SELECT client_id, MAX(encounter_datetime) AS latest_encounter_date, pss_issues_identified
                    FROM mamba_fact_encounter_hiv_art_health_education
                    WHERE pss_issues_identified IS NOT NULL
                    GROUP BY client_id) sub_pss_issues_identified ON sub_pss_issues_identified.client_id = patient_id
         LEFT JOIN (SELECT client_id, MAX(encounter_datetime) AS latest_encounter_date, art_preparation
                    FROM mamba_fact_encounter_hiv_art_health_education
                    WHERE art_preparation IS NOT NULL
                    GROUP BY client_id) sub_art_preparation ON sub_art_preparation.client_id = patient_id
         LEFT JOIN (SELECT client_id, MAX(encounter_datetime) AS latest_encounter_date, depression_status
                    FROM mamba_fact_encounter_hiv_art_health_education
                    WHERE depression_status IS NOT NULL
                    GROUP BY client_id) sub_depression_status ON sub_depression_status.client_id = patient_id
         LEFT JOIN (SELECT client_id, MAX(encounter_datetime) AS latest_encounter_date, gender_based_violance
                    FROM mamba_fact_encounter_hiv_art_health_education
                    WHERE gender_based_violance IS NOT NULL
                    GROUP BY client_id) sub_gender_based_violance ON sub_gender_based_violance.client_id = patient_id
         ...
         LEFT JOIN (SELECT client_id, MAX(encounter_datetime) AS latest_encounter_date, ovc_screening
                    FROM mamba_fact_encounter_hiv_art_health_education
                    WHERE ovc_screening IS NOT NULL
                    GROUP BY client_id) sub_ovc_screening ON sub_ovc_screening.client_id = patient_id
         LEFT JOIN (SELECT client_id, MAX(encounter_datetime) AS latest_encounter_date, ovc_assessment
                    FROM mamba_fact_encounter_hiv_art_health_education
                    WHERE ovc_assessment IS NOT NULL
                    GROUP BY client_id) sub_ovc_assessment ON sub_ovc_assessment.client_id = patient_id
         LEFT JOIN (SELECT client_id, MAX(encounter_datetime) AS latest_encounter_date, prevention_components
                    FROM mamba_fact_encounter_hiv_art_health_education
                    WHERE prevention_components IS NOT NULL
                    GROUP BY client_id) sub_prevention_components ON sub_prevention_components.client_id = patient_id
         LEFT JOIN (SELECT client_id, MAX(encounter_datetime) AS latest_encounter_date, linkages_and_refferals
                    FROM mamba_fact_encounter_hiv_art_health_education
                    WHERE linkages_and_refferals IS NOT NULL
                    GROUP BY client_id) sub_linkages_and_refferals ON sub_linkages_and_refferals.client_id = patient_id
         ...
         LEFT JOIN (SELECT client_id,
                    ...
                    GROUP BY client_id) sub_hivdr_results on sub_hivdr_results.client_id = cohort.patient_id
         LEFT JOIN (SELECT client_id,
                           MAX(encounter_date) AS latest_encounter_date,
                           date_hivr_results_recieved_at_facility
                    FROM mamba_fact_encounter_non_suppressed_card
                    WHERE date_hivr_results_recieved_at_facility IS NOT NULL
                    GROUP BY client_id) sub_date_hivr_results_recieved_at_facility
