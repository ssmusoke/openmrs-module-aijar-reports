...
SELECT cohort.patient_id,
       ...
       mfplhbt.result                                                       AS hepatitis_b_test_qualitative,
       syphilis_test_result_for_partner,
       cervical_cancer_screening,
       ...
       hivdr_results,
       date_hivr_results_recieved_at_facility,
       mfplvai.results                                                      as vl_after_iac,
       mfplido.decision                                                     AS decision_outcome,
...
FROM (SELECT DISTINCT e.patient_id, birthdate, dead, gender
      ...
      WHERE mdp.voided = 0
        ...
        AND e.encounter_type_uuid IN
            ('8d5b27bc-c2cc-11de-8d13-0010c6dffd0f', '8d5b2be0-c2cc-11de-8d13-0010c6dffd0f')) cohort
         LEFT JOIN mamba_fact_patients_nationality mfpn ON mfpn.client_id = cohort.patient_id
         LEFT JOIN mamba_fact_patients_marital_status mfpms ON mfpms.client_id = cohort.patient_id
         LEFT JOIN mamba_fact_patients_latest_return_date mfplrd ON mfplrd.client_id = cohort.patient_id
         LEFT JOIN mamba_fact_patients_latest_current_regimen mfplcr ON mfplcr.client_id = cohort.patient_id
         LEFT JOIN mamba_fact_patients_latest_adherence mfpla ON mfpla.client_id = cohort.patient_id
         LEFT JOIN mamba_fact_patients_latest_arv_days_dispensed mfpladd ON mfpladd.client_id = cohort.patient_id
         LEFT JOIN mamba_fact_patients_latest_viral_load mfplvl ON mfplvl.client_id = cohort.patient_id
         LEFT JOIN mamba_fact_patients_latest_viral_load_ordered mfplvlo ON mfplvlo.client_id = cohort.patient_id
         LEFT JOIN mamba_fact_patients_latest_advanced_disease mfplad ON mfplad.client_id = cohort.patient_id
         LEFT JOIN mamba_fact_patients_latest_family_planning mfplfp ON mfplfp.client_id = cohort.patient_id
         LEFT JOIN mamba_fact_patients_latest_nutrition_assesment mfplna ON mfplna.client_id = cohort.patient_id
         LEFT JOIN mamba_fact_patients_latest_nutrition_support mfplnsmfplns
                   ON mfplnsmfplns.client_id = cohort.patient_id
         LEFT JOIN mamba_fact_patients_latest_hepatitis_b_test mfplhbt ON mfplhbt.client_id = cohort.patient_id
         LEFT JOIN mamba_fact_patients_latest_tb_status mfplts ON mfplts.client_id = cohort.patient_id
         LEFT JOIN mamba_fact_patients_latest_tpt_status mfplts2 ON mfplts2.client_id = cohort.patient_id
         LEFT JOIN mamba_fact_patients_latest_who_stage who_stage ON who_stage.client_id = cohort.patient_id
         LEFT JOIN mamba_fact_patients_latest_regimen_line mfplrl ON mfplrl.client_id = cohort.patient_id
         LEFT JOIN mamba_fact_patients_latest_iac_sessions mfplis ON mfplis.client_id = cohort.patient_id
         LEFT JOIN mamba_fact_patients_latest_vl_after_iac mfplvai ON mfplvai.client_id = cohort.patient_id
         LEFT JOIN mamba_fact_patients_latest_iac_decision_outcome mfplido ON mfplido.client_id = cohort.patient_id
         LEFT JOIN mamba_fact_patients_latest_index_tested_children mfplitc ON mfplitc.client_id = cohort.patient_id
         LEFT JOIN mamba_fact_patients_latest_index_tested_children_status mfplitcs
                   ON mfplitcs.client_id = cohort.patient_id
         LEFT JOIN mamba_fact_patients_latest_index_tested_partners mfplitp ON mfplitp.client_id = cohort.patient_id
         LEFT JOIN mamba_fact_patients_latest_index_tested_partners_status mfplitps
                   ON mfplitps.client_id = cohort.patient_id
         ...
         LEFT JOIN (SELECT a.client_id, hivdr_sample_collected
                   ...
                   ON sub_hivdr_sample_collected.client_id = cohort.patient_id
